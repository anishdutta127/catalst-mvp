/**
 * LLM Response Parser for the Shortcut Path (Path C).
 *
...
function extractOrbs(text: string): [string, string, string] {
  ...
  const m = text.match(re);
  if (!m) return ['Vision', 'Grit', 'Analysis'];
...
export function parseLLMResponse(text: string): Partial<ForgeProfile> {
  ...
  const s1 = decision === 'data-first' ? '📊' : decision === 'people-first' ? '📞' : decision === 'vision-first' ? '📢' : '🔧';
  const s2 = comp === 'validate' ? '🎯' : comp === 'differentiate' ? '🔍' : comp === 'loyalty' ? '🤝' : '⚡';
  const s3 = team === 'large-team' ? '👤' : work === 'seller' ? '📈' : work === 'builder' ? '💻' : '🔬';
  ...
  return {
    blot_responses: [blot1, blot2, blot3],
    blot_response_times: [2000, 2000, 2000],
    word_responses: [powerWord, togetherWord, buildWord, riskWord],
    word_response_times: [1500, 1500, 1500, 1500],
    scenario_responses: [s1, s2, s3],
    scenario_response_times: [2000, 2000, 2000],
    crystal_orbs: orbs,
    crystal_selection_order: [0, 1, 2],
    crystal_selection_times: [1000, 1000, 1000],
